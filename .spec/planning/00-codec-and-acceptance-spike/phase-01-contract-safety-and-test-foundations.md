...
- [ ] Every unsafe case is rejected before source execution, compilation, or mutation.
...
- [ ] Repeated runs produce byte-identical outputs and stable finding codes.

- [ ] Task 1.4.1 is complete with a single documented command for the safe suite.
...
- [ ] The validator accepts the complete package and emits a deterministic summary.
...
- [ ] Each seeded defect produces a specific validation failure and no admitted state.

- [ ] Task 1.4.2 is complete with all negative evidence cases passing.
...
- [ ] Contract and type documentation links are recorded.
- [ ] Isolation and evidence-protocol test commands pass from a clean checkout.
- [ ] Property-test dependency and deterministic builders are present.
- [ ] No test accesses or requires an original LecoWin2 or SBT tree.
- [ ] Phase status and the stream index are updated from `Planned` only when all evidence is linked.
